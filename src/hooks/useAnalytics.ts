...
export function useAnalytics() {
  ...
  const fetch = useCallback(async () => {
    ...
    const [
      ...
      supabase.from("chats").select("id, title, saved, created_at, updated_at").eq("user_id", user.id).order("created_at", { ascending: false }),
      supabase.from("chat_messages").select("id, chat_id, role, created_at"),
