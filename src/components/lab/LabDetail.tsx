...
// ── Auto-detect CSV / JSON from content when language tag is missing ─────────
function detectLanguage(code: string, declaredLanguage?: string): string {
  ...
  // CSV detection: first line has commas, second line has same number of commas
  const lines = trimmed.split("\n").filter(l => l.trim());
  if (lines.length >= 2) {
    const firstCommas = (lines[0].match(/,/g) || []).length;
    const secondCommas = (lines[1].match(/,/g) || []).length;
    if (firstCommas >= 2 && firstCommas === secondCommas) {
      return "csv";
    }
  }
...
function CodeBlock({ language, code }: { language?: string; code: string }) {
  ...
  const handleDownload = () => {
    const ext =
      detectedLang === "csv"  ? "csv"  :
      detectedLang === "json" ? "json" :
      detectedLang === "sql"  ? "sql"  :
      detectedLang === "python" || detectedLang === "py" ? "py" :
      detectedLang === "javascript" || detectedLang === "js" ? "js" :
      detectedLang === "bash" || detectedLang === "sh" ? "sh" :
      "txt";

    const mime =
      detectedLang === "csv"  ? "text/csv" :
      detectedLang === "json" ? "application/json" :
      "text/plain";
  ...
  // Show download for data formats
  const showDownload = detectedLang === "csv" || detectedLang === "json";
        ...
        <div className="flex items-center gap-1">
          {showDownload && (
            <button
              ...
              title={`Download as .${detectedLang}`}
            >
